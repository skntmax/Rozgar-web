...
import Swiggys from '../../../components/companies/Swiggy/Swiggy'
...
export default class Swiggy extends Component {
  ...
  render() {
    ...
    return (
      <React.Fragment>
         <Helmet>
                 <title title={capFirstLetterInSentence(window.location.pathname.replace('/', '').split('/').pop().split('-').join(' ')) + " Overview  – Jobs, Swiggy - Rozgar.com"}>{capFirstLetterInSentence(window.location.pathname.replace('/', '').split('/').pop().split('-').join(' ')) + " Overview  – Jobs, Work Culture - Rozgar.com"}</title>
